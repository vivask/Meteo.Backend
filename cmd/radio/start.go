...
var (
	...
	startCmd   = &cobra.Command{
		Use:   "start",
		Short: "start radio",
		Long:  `start radio, default rest port is 19000`,
		Run:   startRadio,
	}
	enablePprof bool
)
...
func init() {
	...
	startCmd.PersistentFlags().Int("port", 19000, "Port to run Application server on")
...
func startRadio(cmd *cobra.Command, agrs []string) {

	log.SetLogger(config.Default.Radio.Title, config.Default.Radio.LogLevel)
	...
	// run the rest server
	var address = config.Default.Radio.Api.Bind
	var port = config.Default.Radio.Api.Port
	if port > 0 {
	...
	go func() {
		...
		caCert, err := os.ReadFile(config.Default.Radio.Api.Ca)
		if err != nil {
		...
		log.Infof("starting %s server on: https://%s", config.Default.Radio.Title, address)
		if err := srv.ListenAndServeTLS(config.Default.Radio.Api.Crt, config.Default.Radio.Api.Key); err != nil && err != http.ErrServerClosed {
	...
	<-quit
	log.Infof("Shutdown %s Server ...", config.Default.Radio.Title)

	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancel()
	if err := srv.Shutdown(ctx); err != nil {
		log.Errorf("%s Server Shutdown error: %s", config.Default.Radio.Title, err)
	}
	// catching ctx.Done(). timeout of 1 seconds.
	<-ctx.Done()
	log.Infof("%s Server exiting", config.Default.Radio.Title)
}
