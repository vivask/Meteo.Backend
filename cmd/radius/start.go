...
var (
	...
	startCmd   = &cobra.Command{
		Use:   "start",
		Short: "start radius",
		Long:  `start radius, default rest port is 15000`,
		Run:   startRadius,
	}
	enablePprof       bool
...
func init() {
	...
	startCmd.PersistentFlags().Int("port", 15000, "Port to run Application server on")
...
func startRadius(cmd *cobra.Command, agrs []string) {

	log.SetLogger(config.Default.Radius.Title, config.Default.Radius.LogLevel)

	WaitForStartCluster()

	log.Info("Starting radius...")

	db, err := gorm.Open(postgres.Open(getDbUrl(config.Default.Radius.DbLink)))
	if err != nil {
	...
	// run the rest server
	var address = config.Default.Radius.Api.Bind
	var port = config.Default.Radius.Api.Port
	if port > 0 {
	...
	go func() {
		...
		caCert, err := os.ReadFile(config.Default.Radius.Api.Ca)
		if err != nil {
		...
		log.Infof("starting %s server on: https://%s", config.Default.Radius.Title, address)
		if err := srv.ListenAndServeTLS(config.Default.Radius.Api.Crt, config.Default.Radius.Api.Key); err != nil && err != http.ErrServerClosed {
	...
	<-quit
	log.Infof("Shutdown %s Server ...", config.Default.Radius.Title)

	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancel()
	if err := srv.Shutdown(ctx); err != nil {
		log.Errorf("%s Server Shutdown error: %s", config.Default.Radius.Title, err)
	}
	// catching ctx.Done(). timeout of 1 seconds.
	<-ctx.Done()
	log.Infof("%s Server exiting", config.Default.Radius.Title)
}
