...
var (
	...
	startCmd   = &cobra.Command{
		Use:   "start",
		Short: "start schedule",
		Long:  `start schedule, default rest port is 12000`,
		Run:   startSchedule,
	}
	enablePprof       bool
...
func startSchedule(cmd *cobra.Command, agrs []string) {
	...
	<-quit
	log.Infof("Shutdown %s Server ...", config.Default.Schedule.Title)
	...
	<-ctx.Done()
	log.Infof("%s Server exiting", config.Default.Schedule.Title)
}
