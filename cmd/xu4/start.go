...
var (
	...
	startCmd   = &cobra.Command{
		Use:   "start",
		Short: "start xu4",
		Long:  `start xu4, default rest port is 15000`,
		Run:   startXU4,
	}
	enablePprof       bool
...
func startXU4(cmd *cobra.Command, agrs []string) {
	...
	// run the rest server
	var address = config.Default.Xu4.Bind
	var port = config.Default.Xu4.Port
	if port > 0 {
	...
	go func() {
		log.Infof("Bind %s Server on: %s:%d", config.Default.Xu4.Title, address, port)
